...
export const useQueryStore = create<QueryState>()(
  persist(
    (set) => ({
      ...
      updateActiveQueryCode: (code) => set((state) => ({
        queries: state.queries.map(q => q.id === state.activeQueryId ? { ...q, code } : q)
      })),

      requestRun: () => set((state) => ({ runRequested: state.runRequested + 1 })),
      ...
      removeQuery: (id) => set((state) => {
        ...
        return {
          queries: filtered,
          activeQueryId: state.activeQueryId === id ? filtered[0].id : state.activeQueryId
        };
      })
    }),
    {
      name: 'query-storage',
      partialize: (state) => ({ queries: state.queries, activeQueryId: state.activeQueryId }),
    }
  )
);
