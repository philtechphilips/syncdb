...
export default function BackupRestorePage() {
    ...
    const [backupFormat, setBackupFormat] = useState<'sql' | 'csv' | 'json'>('sql');
    const [restoreFormat, setRestoreFormat] = useState<'sql' | 'csv' | 'json'>('sql');

    const handleBackup = async () => {
        if (!selectedCluster) return toast.error("Please select a cluster first");
        
        setIsBackingUp(true);
        const t = toast.loading(`Generating ${backupFormat.toUpperCase()} backup...`);
        
        try {
            const data = await backup(selectedCluster.id, backupFormat);
            
            // Download the file
            const blobContent = backupFormat === 'sql' ? data.content : JSON.stringify(data, null, 2);
            const blob = new Blob([blobContent], { type: backupFormat === 'json' ? "application/json" : "text/plain" });
    ...
    if (!selectedCluster) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-zinc-500">
                ...
                <p className="text-sm font-medium">Please select a cluster to use Backup & Restore</p>
            </div>
        );
    }

    const formats = [
        { id: 'sql', label: 'SQL Dump', icon: Database, description: 'Full schema and data. Best for migrations.' },
        { id: 'json', label: 'JSON Data', icon: FileJson, description: 'Table structures. Great for API portability.' },
        { id: 'csv', label: 'CSV Files', icon: FileText, description: 'Comma separated values. Ideal for snapshots.' },
    ];
            ...
            <div className="max-w-4xl mx-auto space-y-10">
                ...
                <div className="mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {mode === 'backup' ? (
                        ...
                        <div className="group rounded-3xl border border-white/5 bg-zinc-900/40 p-10 flex flex-col gap-10 transition-all hover:bg-zinc-900/60 hover:border-white/10 relative overflow-hidden">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {formats.map((f) => (
                                    ...
                                    >
                                        <div className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${backupFormat === f.id ? 'bg-primary text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                                            <f.icon className="h-5 w-5" />
                                        </div>
                                        <div className="flex flex-col flex-1">
                                            <span className={`text-[10px] font-black uppercase tracking-widest ${backupFormat === f.id ? 'text-primary' : 'text-zinc-200'}`}>{f.label}</span>
                                            <p className="text-[10px] text-zinc-500 mt-1 leading-tight">{f.description}</p>
                                        </div>
                                        {backupFormat === f.id && <CheckCircle2 className="h-4 w-4 text-primary absolute top-4 right-4" />}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="group rounded-3xl border border-white/5 bg-zinc-900/40 p-10 flex flex-col gap-10 transition-all hover:bg-zinc-900/60 hover:border-white/10 relative overflow-hidden">
                            ...
                            <div className="space-y-3">
                                <h2 className="text-xl font-black text-white">System Restore</h2>
                                <p className="text-zinc-500 text-sm font-medium leading-relaxed max-w-xl">
                                    Recover your database to a previous state by uploading a backup. 
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {formats.map((f) => (
                                    ...
                                    >
                                        <div className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${restoreFormat === f.id ? 'bg-primary text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                                            <f.icon className="h-5 w-5" />
                                        </div>
                                        <div className="flex flex-col flex-1">
                                            <span className={`text-[10px] font-black uppercase tracking-widest ${restoreFormat === f.id ? 'text-primary' : 'text-zinc-200'}`}>{f.label}</span>
                                            <p className="text-[10px] text-zinc-500 mt-1 leading-tight">{f.description}</p>
                                        </div>
                                        {restoreFormat === f.id && <CheckCircle2 className="h-4 w-4 text-primary absolute top-4 right-4" />}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="relative mt-4">
                                <input 
                                    ...
                                    id="restore-file" 
                                    className="hidden" 
                                    accept={restoreFormat === 'sql' ? '.sql' : restoreFormat === 'json' ? '.json' : '.csv'}
                                ...
                                >
                                    {isRestoring ? (
                                        ...
                                        <>
                                            ...
                                            <span className="text-[13px] font-black uppercase tracking-widest text-white">Select & Restore File</span>
                                        </>
                                    )}
                                </label>
                            </div>
                ...
                <div className="bg-zinc-900/80 border border-white/5 rounded-2xl p-6 flex gap-5 backdrop-blur-md">
                    ...
                    <div className="space-y-1">
                        <span className="text-[11px] font-black text-primary uppercase tracking-widest">Workspace Context</span>
                        <p className="text-[13px] text-zinc-400 font-medium leading-relaxed">
                            Backup/Restore is currently targeting <span className="text-white font-bold">{selectedCluster.name}</span> ({selectedCluster.type}). 
                            All generated files are encrypted in transit and never stored on our intermediate servers.
                        </p>
                    </div>
                </div>
            </div>
