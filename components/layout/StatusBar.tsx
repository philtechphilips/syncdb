...
const StatusBar = () => {
    const { selectedCluster, selectedTable, isDataLoading, error } = useClusterStore();
    ...
    return (
        <div className="flex h-8 w-full shrink-0 items-center justify-between border-t border-white/5 bg-[#021016] px-6 text-[11px] font-medium text-zinc-400 backdrop-blur-md">
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-2 group cursor-default">
                    <DatabaseIcon className={`h-3.5 w-3.5 ${selectedCluster ? 'text-primary' : 'text-zinc-500'}`} />
                    <span className={selectedCluster ? 'text-zinc-200' : 'text-zinc-500'}>{dbDisplay}</span>
                </div>
            ...
            <div className="flex items-center gap-6">
                {isDataLoading ? (
                    ...
                    <div className="flex items-center gap-2 text-rose-500">
                        ...
                        <span className="font-bold uppercase tracking-widest text-[9px] line-clamp-1 max-w-[200px]">{error}</span>
                    </div>
                ) : (
                    <div className="flex items-center gap-2 text-emerald-400">
                        <CheckCircle2 className="h-4 w-4" />
                        <span className="font-bold uppercase tracking-widest text-[9px]">Operational</span>
                    </div>
                )}
