...
const Sidebar = ({
    ...
    const mockTables = [
        { name: "users", rows: "5k" },
        { name: "orders", rows: "12k" },
        { name: "products", rows: "800" },
        { name: "analytics", rows: "45k" },
        { name: "logs", rows: "1.2M" },
    ];
            ...
            <div className="p-4 border-b border-border relative">
                ...
                >
                    <div className="h-8 w-8 rounded bg-primary flex items-center justify-center text-primary-foreground font-black text-xs shadow-lg shadow-primary/20 transition-transform group-hover:scale-105">
                        {mockConnections.find(c => c.name === activeConnection)?.icon || "M"}
                    </div>
                    <div className="flex flex-col">
                        <span className="text-xs font-bold text-foreground truncate w-24">{activeConnection}</span>
                        <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter truncate w-24">
                            {mockConnections.find(c => c.name === activeConnection)?.cluster}
                        </span>
                    </div>
                ...
                {isConnectionDropdownOpen && (
                    <div className="absolute left-4 top-[calc(100%-8px)] z-50 w-[calc(100%-32px)] bg-zinc-900 border border-white/5 rounded-xl shadow-2xl py-2 animate-in fade-in zoom-in-95 duration-150">
                        <div className="px-3 py-1.5 mb-1.5 border-b border-white/5">
                            <span className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">Switch Connection</span>
                        </div>
                        <div className="max-h-60 overflow-y-auto scrollbar-hide">
                            {mockConnections.map((conn) => (
                                <button
                                    key={conn.name}
                                    onClick={() => {
                                        setActiveConnection(conn.name);
                                        setIsConnectionDropdownOpen(false);
                                    }}
                                    className={`flex w-full items-center gap-3 px-3 py-2 text-xs font-semibold transition-colors hover:bg-white/5 ${activeConnection === conn.name ? 'text-primary' : 'text-zinc-400'}`}
                                >
                                    <div className={`h-6 w-6 rounded flex items-center justify-center text-[10px] font-black ${activeConnection === conn.name ? 'bg-primary/20 text-primary' : 'bg-zinc-800 text-zinc-500'}`}>
                                        {conn.icon}
                                    </div>
                                    <div className="flex flex-col items-start translate-y-[-1px]">
                                        <span>{conn.name}</span>
                                        <span className="text-[8px] text-zinc-600 font-bold uppercase tracking-tighter">{conn.type}</span>
                                    </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-0.5">
                        ...
                        >
                            ...
                            <span>Tables</span>
                            <span className="ml-auto text-[10px] bg-muted px-1.5 py-0.5 rounded font-mono text-muted-foreground">{mockTables.length}</span>
                        </div>
                            ...
                            <div className="mt-1 flex flex-col animate-in fade-in slide-in-from-top-1 duration-200">
                                {mockTables.map((table) => (
                                    <div
                                        ...
                                        className={`flex items-center gap-3 ml-6 mr-1 px-3 py-1.5 rounded-md text-[11px] font-bold transition-all cursor-pointer group/item hover:bg-white/[0.03] ${selectedTable === table.name ? 'text-primary bg-primary/5' : 'text-zinc-500 hover:text-zinc-300'}`}
                                    >
                                        <div className={`h-1 w-1 rounded-full ${selectedTable === table.name ? 'bg-primary shadow-[0_0_8px_rgba(0,237,100,0.5)]' : 'bg-zinc-800'}`}></div>
                                        <span className="truncate">{table.name}</span>
                                        <span className="ml-auto opacity-0 group-hover/item:opacity-100 text-[8px] font-black text-zinc-600 uppercase tracking-tighter tabular-nums transition-opacity pr-1">
                                            {table.rows}
                                        </span>
                                    </div>
                                ))}
