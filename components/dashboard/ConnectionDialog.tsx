        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="relative w-full max-w-4xl rounded-[2.5rem] bg-background p-10 shadow-3xl border border-white/5 overflow-hidden max-h-[90vh] overflow-y-auto scrollbar-hide">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-10 relative z-10">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {databases.map((db: any) => (
                                <button
                                    ...
                                    className={`flex items-center gap-4 rounded-2xl border p-4 transition-all group relative overflow-hidden ${selectedDb === db.id
                                            ? "border-primary/50 bg-primary/5 shadow-[0_0_20px_rgba(0,237,100,0.1)]"
                                            : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                        }`}
                                >
                                    <div className={`p-2.5 rounded-xl bg-white/5 border border-white/5 group-hover:scale-110 transition-transform ${selectedDb === db.id ? "text-primary" : "text-zinc-500"}`}>
                                        <Database className="h-5 w-5" />
                                    </div>
                                    <div className="flex flex-col items-start">
                                        <span className={`text-sm font-bold ${selectedDb === db.id ? "text-white" : "text-zinc-400"}`}>{db.name}</span>
                        ...
                        <div className="pt-6 space-y-4">
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 ml-1">Identity</span>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 ml-1">Friendly Name</label>
                                <input 
                                    type="text" 
                                    required
                                    value={formData.name}
                        ...
                        {testResult && (
                            <div className={`mt-6 p-4 rounded-2xl border flex items-center gap-3 animate-in slide-in-from-left-2 ${testResult.success ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <div className="glass rounded-[2rem] p-8 border border-white/5 space-y-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-500 ml-1">Database Info</label>
                                <input 
                                    type="text" 
                                    required
                        ...
                        <div className="flex gap-4">
                            ...
                            >
                                {isLoading ? <Loader2 className="h-4 w-4 animate-spin text-primary-foreground" /> : "Establish Connection"}
                            </button>
                        </div>
                    </div>
                </form>
                ...
                <div className="mt-12 flex items-center justify-between gap-6 relative z-10 border-t border-white/5 pt-8">
                    ...
                    <button onClick={onClose} type="button" className="text-xs font-bold text-zinc-600 hover:text-white transition-colors">Discard Configuration</button>
                </div>
            </div>
        </div>
    );
