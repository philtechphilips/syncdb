...
const ERDiagram = () => {
        ...
        const loadSchema = async () => {
            ...
            try {
                ...
                tableNames.forEach(name => {
                    tables[name].forEach(c => {
                        if (c.referencedTable && tables[c.referencedTable]) {
                            newEdges.push({ source: name, target: c.referencedTable, sourceColumn: c.name });
                        }
                    });
                });
                ...
                tableNames.forEach(t => { 
                    tableRanks[t] = 0; 
                    newEdges.filter(e => e.source === t || e.target === t).forEach(e => {
    ...
    const filteredNodes = useMemo(() => {
        ...
        if (search) nodes.forEach(n => { if (n.name.toLowerCase().includes(search.toLowerCase())) matchingIds.add(n.id); });
    ...
    const exportDiagram = async (format: 'png' | 'jpeg' | 'pdf') => {
        ...
        try {
            ...
            const opts = { pixelRatio: 2, bgcolor: '#ffffff', width, height, style: { transform: `translate(${-minX}px, ${-minY}px) scale(1)`, transformOrigin: 'top left', width: `${width}px`, height: `${height}px` } };
            const dataUrl = format === 'jpeg' ? await toJpeg(contentRef.current, opts) : await toPng(contentRef.current, opts);
        ...
        } catch (err) { toast.error("Memory limit exceeded for large schema.", { id: toastId }); }
    ...
    const getColumnPoint = (nodeId: string, colName?: string) => {
        ...
        if (!colName || !node.isExpanded) return { x: node.x + 208, y: node.y + 20 };
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        if (draggingNode) {
            setNodes(prev => prev.map(n => n.id === draggingNode.id ? { ...n, x: (e.clientX / zoom) - draggingNode.offsetX, y: (e.clientY / zoom) - draggingNode.offsetY } : n));
        } else if (panning) {
            setCanvasOffset({ x: e.clientX - panning.startX, y: e.clientY - panning.startY });
        }
    };
    ...
    return (
        ...
        >
            <DiagramToolbar 
                ...
                onResetView={() => { setCanvasOffset({ x: 0, y: 0 }); setZoom(1); }}
                onToggleFullscreen={() => setIsFullscreen(!isFullscreen)}
                onToggleAll={(exp) => setNodes(prev => prev.map(n => ({ ...n, isExpanded: exp })))}
            ...
            <div
                ...
                onMouseDown={(e) => e.button === 0 && setPanning({ startX: e.clientX - canvasOffset.x, startY: e.clientY - canvasOffset.y })}
                onWheel={handleWheel}
                ref={diagramRef}
            >
                {isLoading && (
                ...
                <div
                    ...
                    style={{ backgroundImage: `radial-gradient(circle, #277955 1px, transparent 1px)`, backgroundSize: `${40 * zoom}px ${40 * zoom}px`, backgroundPosition: `${canvasOffset.x}px ${canvasOffset.y}px` }}
                ></div>
                    ...
                    {filteredNodes.map((node) => (
                        <TableNode 
                            ...
                            onMouseDown={(e) => { 
                                ...
                                setDraggingNode({ id: node.id, offsetX: (e.clientX / zoom) - node.x, offsetY: (e.clientY / zoom) - node.y });
                                e.stopPropagation(); 
                            }}
                            isActive={activeTableId === node.id}
                            onToggleExpand={() => setNodes(prev => prev.map(n => n.id === node.id ? { ...n, isExpanded: !n.isExpanded } : n))}
            ...
            <DiagramOverlay activeTableId={activeTableId} onClearFocus={() => setActiveTableId(null)} />

            {isFullscreen && (
                <button onClick={() => setIsFullscreen(false)} className="absolute top-6 right-8 z-[110] px-4 py-2 rounded-lg bg-red-500/10 text-red-500 border border-red-500/20 text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-xl">
                    Close [Esc]
                </button>
            )}
        </div>
