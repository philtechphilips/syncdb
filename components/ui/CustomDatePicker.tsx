        ...
        <div className="relative" ref={containerRef}>
            ...
            >
                <div className="absolute left-4 top-1/2 -translate-y-1/2">
                    {isDateTime ? <Clock className="h-4 w-4 text-primary/50" /> : <Calendar className="h-4 w-4 text-primary/50" />}
                </div>
                <span className={value ? 'text-white' : 'text-zinc-500'}>
                    {value ? (isDateTime ? new Date(value).toLocaleString() : new Date(value).toLocaleDateString()) : `Select ${isDateTime ? 'Date & Time' : 'Date'}...`}
                </span>
            </button>
                ...
                <div className="absolute top-full mt-2 left-0 z-[210] bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl p-4 animate-in fade-in zoom-in-95 duration-150 min-w-[300px]">
                    ...
                    <div className="grid grid-cols-7 gap-1 mb-2">
                        {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(d => (
                            <div key={d} className="text-[10px] font-black text-zinc-600 text-center uppercase py-1">{d}</div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        ...
                        {Array.from({ length: daysInMonth(viewDate.getFullYear(), viewDate.getMonth()) }).map((_, i) => {
                            ...
                            const isSelected = (selectedDate && 
                                ...
                                selectedDate.getFullYear() === viewDate.getFullYear()) || false;
                    ...
                    {isDateTime && (
                        <div className="mt-4 pt-4 border-t border-white/5 flex flex-col gap-3">
                            <div className="flex items-center justify-between">
                                ...
                                {value && (
                                    <span className="text-[10px] font-bold text-primary">{new Date(value).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                )}
                            </div>
                            <input 
                                type="time"
                                value={value ? new Date(value).toISOString().slice(11, 16) : "12:00"}
                                onChange={(e) => {
