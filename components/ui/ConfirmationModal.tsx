...
const ConfirmationModal = () => {
    const { isOpen, title, message, onConfirm, onCancel, confirmLabel, cancelLabel, type, isAlert, confirmValue, close } = useModalStore();
    ...
    const colorClass = type === 'danger' ? 'text-red-500 bg-red-500/10 border-red-500/20' : 
                ...
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                    ...
                    >
                        <div className="px-6 py-8 flex flex-col items-center text-center">
                            <div className={`h-16 w-16 rounded-2xl ${colorClass} border flex items-center justify-center mb-6`}>
                                <Icon className="h-8 w-8" />
                            </div>
                            
                            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2">{title}</h3>
                            <p className="text-sm font-bold text-zinc-500 leading-relaxed mb-6">{message}</p>
                            
                            {confirmValue && (
                                <div className="w-full mb-8 space-y-3">
                                    <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest text-left px-1">
                                        Type <span className="text-white font-mono bg-white/5 py-0.5 px-1.5 rounded border border-white/5 select-all">{confirmValue}</span> to confirm
                                    </p>
                                    <input 
                                        type="text" 
                                        autoFocus
