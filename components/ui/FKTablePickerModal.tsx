...
import { X, Search, Loader2, Database, ChevronLeft, ChevronRight, Check } from "lucide-react";
...
const FKTablePickerModal = ({
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (columns.length === 0) {
                const colRes = await api.get(`/v1/clusters/${clusterId}/tables/${referencedTable}/columns`);
            ...
            <div className="relative w-full max-w-6xl h-[85vh] bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-white uppercase tracking-tight">Select from {referencedTable}</h3>
                            <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Target Column: {referencedColumn}</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-hidden relative flex flex-col">
                    {isLoading ? (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-zinc-900/50 backdrop-blur-sm z-10">
                            ...
                            <span className="text-xs font-black text-zinc-500 uppercase tracking-widest">Loading Records...</span>
                        </div>
                    ) : null}
                    ...
                    <div className="flex-1 overflow-auto scrollbar-hide">
                        <table className="w-full text-left border-collapse min-w-max">
                            <thead className="sticky top-0 z-20 bg-zinc-900/90 backdrop-blur-md shadow-sm">
                                <tr>
                                    <th className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest border-b border-white/5">Selection</th>
                                    {columns.map(col => (
                                        <th key={col.name} className="px-6 py-4 text-[10px] font-black text-zinc-500 uppercase tracking-widest border-b border-white/5">
                                            {col.name}
                                        </th>
                                    ))}
                                </tr>
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-primary border-primary' : 'border-white/10 group-hover:border-white/30'}`}>
                                                    {isSelected && <Check className="h-3 w-3 text-primary-foreground stroke-[4]" />}
                                                </div>
                                            </td>
                                            {columns.map(col => (
                                                <td key={col.name} className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`text-sm font-medium ${isSelected ? 'text-white' : 'text-zinc-400 group-hover:text-zinc-200'}`}>
                                                        {row[col.name] === null ? <span className="text-[10px] font-black text-zinc-700 uppercase italic">NULL</span> : String(row[col.name])}
                                                    </span>
                                                </td>
                                            ))}
                                        </tr>
                        ...
                        {filteredData.length === 0 && !isLoading && (
                            <div className="flex flex-col items-center justify-center py-20 gap-4">
                                ...
                                <div className="text-center">
                                    <p className="text-sm font-medium text-zinc-400">No records found matching your search</p>
                                    <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest mt-1">Try adjusting your filters</p>
                                </div>
                            </div>
                        )}
                    </div>
